...
class NisraUpdateCheckService:
    ...
    def check_nisra_files_have_updated(self) -> str:
        questionnaire_names_in_blaise = self._blaise_service.get_names_of_questionnaire_in_blaise(self._config.survey_type)
        questionnaire_dates_in_bucket = self._bucket_service.get_questionnaire_modified_dates(self._config.bucket_file_type)
        ...
        for questionnaire_name in questionnaire_names_in_blaise:
            ...
            self.notify_if_questionnaire_has_not_been_updated(questionnaire_name, questionnaire_dates_in_bucket[questionnaire_name])

        return "Done"

    def notify_questionnaire_is_missing_in_the_bucket(self, questionnaire_name: str) -> None:
        logging.info(f"{questionnaire_name} is missing in the NISRA bucket, sending notification")
    ...
    def notify_if_questionnaire_has_not_been_updated(self, questionnaire_name: str, date_modified: datetime) -> None:
        if self.questionnaire_has_not_updated_within_max_hours(date_modified):
            logging.info(f"{questionnaire_name} has a modified date {date_modified} past {self._config.max_hours_since_last_update} " f"hours, sending notification")
