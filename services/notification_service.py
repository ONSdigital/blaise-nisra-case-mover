...
class NotificationService:
    ...
    def send_email_notification(self, message: Dict[str, str], template_id: str) -> None:
        try:
            logging.info(f"Sending email notification {message} to {self._email_address}")
            ...
            logging.error(f"NotificationService: Error when sending email via GOV.UK Notify API - {error}")
            raise error
